...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        # 将输入张量进行embedding操作, 得到的是一个二维张量，并使其形状变为(1,1,-1),第一个1代表扩展维度，-1代表自动计算维度（即embedding）
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    """训练函数, 输入参数有8个, 分别代表input_tensor：源语言输入张量，target_tensor：目标语言输入张量，encoder, decoder：编码器和解码器实例化对象
       encoder_optimizer, decoder_optimizer：编码器和解码器的优化器，criterion：损失函数计算方法，max_length：句子的最大长度"""
    ...
    for ei in range(input_length):
        ...
        # 将每次获得的输出encoder_output(三维张量), 使用[0, 0]降两维变成向量依次存入到encoder_outputs（变成一维向量）
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    ...
    with torch.no_grad():
        ...
        for di in range(max_length):
            ...
            # 最后将本次预测的索引降维并分离赋值给decoder_input，以便下次进行预测.不能使用teacher_forcing，teacher_forcing只用在训练上。
            decoder_input = topi.squeeze().detach()
